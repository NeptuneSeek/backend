...
async def artisan_details(artisan: dict, client: AsyncClient):
    url = artisan["url"]
    try:
        response = await client.get(url)
        response.raise_for_status()
        ...
        artisan.update({"rating": rating,  "about": about,"reviews": reviews_data})
    except Exception as e:
        print(f"Error fetching details for {artisan['name']}: {e}")
        artisan.update({"rating": "N/A", "about": "N/A", "reviews": []})
